...
class NaiveRetriever:
  ...
  def _retrieve_by_str(self, query: str) -> str:
    retrieved_chunks_list = self._wcs_client_adapter.retrieve_by_query(query, 5)
    formatted_chunks = "\n\n".join(chunk for chunk in retrieved_chunks_list)
    return formatted_chunks
  
  def _retrieve_by_vector(self, query: str) -> str:
    query_vector = self._vectorize(query)
    retrieved_chunks_list = self._wcs_client_adapter.retrieve_by_query_vector(query_vector, 5)
    formatted_chunks = "\n\n".join(chunk for chunk in retrieved_chunks_list)
    return formatted_chunks
